...
Sentry.init({
  dsn: process.env.SENTRY_DSN || process.env.LOGGER_URL,
  environment: process.env.SENTRY_ENV || process.env.APP_ENVIRONMENT
});

const getParameterValue = parameterResponse =>
  parameterResponse.Parameter.Value;
const getSSMParameterValue = Name =>
...
const prepareHeaders = (headers, cors) =>
...
const getErrorResponseBody = (message, type) =>
  log(`Error response of type "${type}" with message: "${message}"`) || {
    error: {
      message,
      type
    }
...
const requestOAuth2AccessToken = (endpoint, clientId, clientSecret) =>
  axios.post(endpoint, qs.stringify({ grant_type: "client_credentials" }), {
...
const handleBackendException = err => {
  ...
  return getResponseObject(
    errResponse.status,
    {},
    getErrorResponseBody(errResponse.data.message, "Backend")
  );
};
